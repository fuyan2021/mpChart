...
import com.example.mpandroiddemo.databinding.ActivityChartBinding;
...
//        mLineChart = binding.chart;
        // *************************数据转换********************* //
        float[] dataObjects = {1, 4, 3, 1, 5, 2, 7, 4, 5, 1, 3, 2, 6};
        ...
        dataSet.setColors(Color.GREEN); // 每个点之间线的颜色，还有其他几个方法，自己看
...
//        mLineChart.setContentDescription("××表");   // 右下角的描述文本,测试并不显示
        ...
        mLineChart.setOnChartGestureListener(new OnChartGestureListener() { // 手势监听器
            ...
            public void onChartTranslate(MotionEvent me, float dX, float dY) {
                // 移动
                Log.d(TAG, "onChartTranslate: " + dX + "-" + dY);
            }
        });
        ...
        mLineChart.setMaxHighlightDistance(10);    // 最大高亮距离（dp）,点击位置距离数据点的距离超过这个距离不会高亮，默认500dp
        ...
        dataSet.setHighLightColor(Color.BLUE); // 高亮颜色,默认RGB(255, 187, 115)
        ...
        xAxis.enableGridDashedLine(20, 10, 1);    // 网格线为虚线，lineLength，每段实线长度,spaceLength,虚线间隔长度，phase，起始点（进过测试，最后这个参数也没看出来干啥的）
...
//        mLineChart.animateXY(3000, 3000);   // 上面两个的结合
        mLineChart.animateX(1000, Easing.EaseInQuad); // 动画播放随时间变化的速率，有点像插值器。后面这个有的不能用


        // **************************所有数据样式************************************ //
        mLineChart.setMarker(new XYLine(this));    // 点击数据点显示的pop，有俩默认的，MarkerImage：一张图片，MarkerView:一个layout布局,也可以自己定义.这里这个是我自定义的一个MarkerView。
